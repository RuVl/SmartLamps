...
s.text(
    40,
    72,
    "Красный +5 В · чёрный GND · зелёный данные · синий кнопка",
    size=13,
    color="#495057",
)
...
s.add(
    f"<rect x='{BX + 48}' y='{BY + 18}' width='108' height='96' rx='4' fill='#c7ccd4' stroke='#8d949e'/>"
...
s.add(
    f"<circle cx='{TX + 40}' cy='{TY + 42}' r='22' fill='#e03131' stroke='#a61e1e' stroke-width='3'/>"
...
# VDD through one 1N4007 (anode towards the supply). The red wire from VDD goes right,
# crosses DOUT once and drops to the diode's cathode. C4 sits on the pixel side of the
# diode - between VDD (after the diode) and GND - that is what a decoupling cap is for.
DX = vdd[0] + 60
GX = gnd[0] - 40
s.wire([vdd, (DX, vdd[1]), (DX, vdd[1] + 100)], RED)
s.diode((DX, vdd[1] + 190), (DX, vdd[1] + 100), 'D1 1N4007')
...
for r in range(16):
    for c in range(16):
        ...
        s.add(
            f"<rect x='{x}' y='{y}' width='18' height='18' rx='2' fill='#f1f3f5'/>"
            f"<circle cx='{x + 9}' cy='{y + 9}' r='4' fill='#fff3bf'/>"
        )
s.text(
    MX + 8 * CELL,
    MY - 12,
    "Матрица WS2812B 16×16, 256 LED",
    size=12,
    weight="bold",
    anchor="middle",
)
...
s.add(
    f"<rect x='{MX - 8}' y='{dout[1] - 7}' width='16' height='14' rx='2' fill='#e6c157' stroke='#8a6d1f'/>"
)
s.pin("m.DIN", MX - 8, dout[1])
s.text(
    MX - 18, dout[1] - 7, "DIN", size=10, weight="bold", color="#343a40", anchor="end"
)
MB = MY + 16 * CELL
for name, x in [("5V", MX + 8 * CELL - 35), ("GND", MX + 8 * CELL + 35)]:
    s.add(
        f"<rect x='{x - 7}' y='{MB - 8}' width='14' height='16' rx='2' fill='#e6c157' stroke='#8a6d1f'/>"
    )
    s.pin("m." + name, x, MB + 8)
    s.text(
        x + (-12 if name == "5V" else 12),
        MB + 22,
        name,
        size=10,
        weight="bold",
...
SX, SY = 60, 800
s.add(
    f"<rect x='{SX}' y='{SY}' width='170' height='110' rx='10' fill='#222'/>"
...
s.add(
    f"<rect x='{TBX}' y='{TBY}' width='40' height='140' rx='4' fill='#2b8a3e' stroke='#1e6b30'/>"
)
rows = {}
for i, name in enumerate(["+", "−", "+", "−", "+", "−"]):
    y = TBY + 14 + i * 22
    s.add(
        f"<circle cx='{TBX + 20}' cy='{y}' r='7' fill='#c9ced6' stroke='#6c757d'/>"
...
s.dot(CH1, 720, RED)
s.dot(CH2, 720, BLK)
# pair 2 → pixel: + to the diode's anode, − up the GND wire
s.wire([rows[2], (DX, rows[2][1]), (DX, vdd[1] + 190)], RED)
...
s.note(
    60,
    560,
    ["Кнопка на лампе B пока", "заглушена в прошивке", "(LAMP_BUTTON_DISABLED)."],
    size=11,
)
