...
with schemdraw.Drawing(show=False) as d:
    ...
    mtx = d.add(
        ...
        .anchor("DIN")
        .at((pix.DO.x + 2.5, pix.DO.y))
    )
    d.add(elm.Line().at(pix.DO).to(mtx.DIN).color(GRN))
    c1 = d.add(
        elm.Capacitor2()
        .at((mtx.V1.x, mtx.V1.y - 1.1))
        .right()
        .tox(mtx.G1)
        .label("C1\n1000 µF", loc="bottom")
    )
    ...
    psu = d.add(
        elm.Ic(
            pins=[
                elm.IcPin(name="+", side="top", slot="1/2", anchorname="P"),
    ...
    d.add(
        elm.Label()
        .at((mcu["3V3"].x - 0.5, btn.VCC.y + 3.0))
        .label("Лампа B (ESP8266): схема подключения", fontsize=14, halign="left")
    )
...
cairosvg.svg2png(
    url=str(OUT.with_suffix(".svg")), write_to=str(OUT.with_suffix(".png")), scale=2
)
